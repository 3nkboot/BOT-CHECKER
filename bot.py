...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.document:
        return await update.message.reply_text("❌ لم يتم رفع ملف")
    ...
    try:
        ...
        await status_msg.edit_text(f"✅ تم العثور على {len(valid_cards)} بطاقة\n🔄 جاري الفحص...")
        ...
        for i, card in enumerate(valid_cards):
            ...
            # تحديث التقدم كل 20 بطاقة
            if (i + 1) % 20 == 0:
                await status_msg.edit_text(f"📊 تم فحص {i + 1}/{len(valid_cards)} بطاقة...")
        ...
        if approved > 0:
            result_text += f"\n📈 نسبة النجاح: {(approved/len(valid_cards)*100):.1f}%"
...
def main():
    ...
    try:
        # إنشاء التطبيق - هذا هو الجزء المهم
