...
def check_card(ccx, amount):
    try:
        ...
        mp = MultipartEncoder(fields={
            ...
            'payment-mode': 'paypal-commerce', 'give_first': (None, 'DRGAM'),
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if '|' in text:
        ...
        try:
            ...
            bin_info, bank, country = get_bin_info(text)
            ...
            response_text = (
                f"- - - - - - - - - - - - - - - - - - - - - - -\n"
                f"[ϟ] 𝐂𝐚𝐫𝐝:  <code>{safe_text}</code> 💳\n"
                f"[ϟ] 𝐒𝐭𝐚𝐭𝐮𝐬: <b>{status}</b>\n"
                f"[ϟ] 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞: <code>{safe_result}</code>\n"
                f"- - - - - - - - - - - - - - - - - - - - - - -\n"
                f"[ϟ] 𝐁𝐢𝐧: <code>{bin_info}</code>\n"
                f"[ϟ] 𝐁𝐚𝐧𝐤: <code>{bank}</code>\n"
                f"[ϟ] 𝐂𝐨𝐮𝐧𝐭𝐫𝐲: <code>{country}</code>\n"
                f"- - - - - - - - - - - - - - - - - - - - - - -\n"
                f"[ϟ] T/t : {time_taken} | Proxy : Live ✨\n"
                f"[⌥] 𝐂𝐡𝐞𝐜𝐤𝐞𝐝 𝐛𝐲: @{safe_username} 💪\n"
